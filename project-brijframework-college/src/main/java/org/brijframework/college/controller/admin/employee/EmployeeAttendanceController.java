...
public class EmployeeAttendanceController {
	...
	public String employeeAttendanceRegisterPage(ModelMap model, HttpServletRequest request) throws Exception {
		HttpSession session = request.getSession();
		session.setAttribute("active", "staff");
	...
	public String employeeAttendanceReportPage(ModelMap model, HttpServletRequest request) {
		HttpSession session = request.getSession();
		session.setAttribute("active", "staff");
	...
	public String overallViewaspdf(Model model, @RequestParam("fromDate") String fromDate,
			@RequestParam("toDate") String toDate, @RequestParam("id") int id) throws ParseException {
		model.addAttribute("list", employeeAttendanceService.showEmployeeAttendanceRecordAsPdf(fromDate, toDate, id));
	...
	public String employeeAttendencePage(@RequestParam int id,
			...
			@RequestParam(required = false, defaultValue = "0") int year, Model model) throws Exception {
		model.addAttribute("EMPLOYEESDTO", employeeService.findEmployeeDetails(id));
		model.addAttribute("MAP", employeeAttendanceService.getAttendaceDataForEmp(id, monthId, year));
