...
public class BatchPromoteController {
	...
	public @ResponseBody String getToSession(@RequestParam int sessionId) throws JsonProcessingException {
		return new ObjectMapper().writeValueAsString(sessionService.getToSession(sessionId));
	}

	@RequestMapping(value = "getstudentforbatchtransfer", method = RequestMethod.POST)
	public @ResponseBody String getStudent(@RequestParam int classId, @RequestParam int sectionId,
			@RequestParam(defaultValue = "0", required = false) int sessionId) throws JsonProcessingException {
		return new ObjectMapper().writeValueAsString(
				studentsAdmissionService.getStudentsbyClassandSection(classId, sectionId, sessionId));
	}

	@RequestMapping(value = "transfer-batch.html", method = RequestMethod.POST)
	public String createUpPage(@ModelAttribute FeecategoryAmountDTO feecategoryAmountDTO, ModelMap model) {
		Boolean flag = studentsAdmissionService.transferBatch(feecategoryAmountDTO);
		if (flag == true) {
			model.addAttribute("msg", "Student Transferred Successfully");
		} else {
			model.addAttribute("msg", "Selected Students are already exist in this session.");
		}
		model.addAttribute("FeecategoryAmountDTO", new FeecategoryAmountDTO());
		model.addAttribute("Classes", studentClassesService.getAllClass());
		model.addAttribute("Sessiones", studentClassesService.findAll(Session.class));
		return "batchpromote";
	}
